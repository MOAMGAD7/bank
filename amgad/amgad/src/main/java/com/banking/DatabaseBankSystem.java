...
class database_BankSystem {
    ...
    // Get monthly transactions (NEW FUNCTION)
    ...
    public static double getMonthlyIncome(String username, String month) {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
            ...
            // Get income from transactions (including received transfers)
            ...
            // Get income from cards (excluding first card)
    ...
    public static double getMonthlyExpenses(String username, String month) {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
            ...
            // Get expenses from transactions table (including transfers)
            ...
            // Get outgoing transfers for the month
    ...
    // Deposit money
    // Deposit money with fees

    // Withdraw money
    // Withdraw money with fees
    public static boolean withdraw(String username, double amount) {
        ...
        // Calculate fees (e.g., 2% of the withdrawal amount)
        double feesPercentage = 0.02; // 2% fees
        double fees = amount * feesPercentage;
        double totalAmountAfterFees = amount + fees; // Total amount including fees

        // Check if balance is sufficient after adding fees
        if (current < totalAmountAfterFees) {
            System.out.println("❌ Insufficient funds after fees for user: " + username);
            return false;
        }

        // Update balance with the original amount (withdraw amount)
        boolean balanceUpdated = updateBalance(username, current - amount);
        if (balanceUpdated) {
            int userId = getUserId(username);

            // Record the withdrawal transaction (original amount)
            boolean withdrawalRecorded = recordTransaction(userId, "withdraw", amount);

            // Record the fees as a separate transaction
            ...
            if (withdrawalRecorded && feesRecorded) {
                ...
                // Rollback balance if transaction recording fails
    ...
    public static boolean deposit(String username, double amount) {
        ...
        // Calculate fees (e.g., 2% of the deposit amount)
        double feesPercentage = 0.02; // 2% fees
        double fees = amount * feesPercentage;
        double totalAmountAfterFees = amount + fees; // Amount after adding fees

        // Update balance with the amount (excluding fees for now)
        boolean balanceUpdated = updateBalance(username, current + amount);
        if (balanceUpdated) {
            int userId = getUserId(username);

            // Record the deposit transaction (original amount)
            boolean depositRecorded = recordTransaction(userId, "deposit", amount);
    ...
    // Get all user transactions with details for reporting
    ...
    public static boolean addCard(String username, String cardType, double amount) {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
            ...
            // Check card count and duplicates in a single query
            String checkSql = """
                SELECT 
            ...
            try (PreparedStatement checkStmt = conn.prepareStatement(checkSql)) {
                ...
                try (ResultSet rs = checkStmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        // Add card and record transaction in a single transaction if not first card
                        ...
                        try (PreparedStatement insertStmt = conn.prepareStatement(insertSql)) {
                            ...
                            // If not first card, record as income
                            if (cardCount > 0) {
    ...
    public static int getUserCardCount(String username) {
        ...
        // عد البطاقات النشطة فقط (التي رصيدها أكبر من صفر)
    ...
    // Get user's active cards with their amounts
    public static List<Card> getUserCards(String username) {
        ...
        String sql = "SELECT id, card_type, amount FROM cards WHERE user_id = ? AND amount > 0 ORDER BY id";
        try (Connection conn = DriverManager.getConnection(DB_URL);
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, userId);
            try (ResultSet rs = pstmt.executeQuery()) {
