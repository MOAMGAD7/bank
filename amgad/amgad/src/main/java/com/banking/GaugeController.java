...
public class GaugeController {
    ...
    public void initialize() {
        ...
        // Fetch total balance from database (still needed for other parts)
        double totalBalance = database_BankSystem.getTotalBalance(username);

        // Declare today once for the entire method
        LocalDate today = LocalDate.now();

        // Fetch total withdrawals and deposits for the current month from database
        ...
        // Fetch withdrawals and deposits using correct types
        List<Object[]> withdrawalData = database_BankSystem.getMonthlyTransactions(username, "withdraw", start, endDate);
        List<Object[]> depositData = database_BankSystem.getMonthlyTransactions(username, "deposit", start, endDate);
        ...
        for (Object[] data : withdrawalData) {
            totalWithdrawals += (Double) data[1];
        }

        // Sum deposits
        for (Object[] data : depositData) {
            totalDeposits += (Double) data[1];
        }

        // Calculate total transactions (deposit + withdraw)
        double totalTransactions = totalDeposits + totalWithdrawals;
        if (totalBalance <= 0) {
            totalBalance = 1.0; // Avoid division by zero
        }
        ...
        // Calculate Earning as a value (15% of totalBalance) - unchanged
        double earningValue = totalBalance * (15.0 / 100);
        double earningPercentage = (totalBalance > 0) ? (earningValue / totalBalance) * 100 : 0.0;

        System.out.println("Total Deposits (Database): " + totalDeposits);
        System.out.println("Total Withdrawals (Database): " + totalWithdrawals);
        ...
        // Fetch payment data from database - unchanged
        List<Object[]> categoryData = database_BankSystem.getPaymentTotalsByCategory(userId);

        // Add data to Donut Chart with custom colors - unchanged
        ...
        // Update legend dynamically - unchanged
        ...
        addTypingEffect(progressBar1, String.format("Withdrawals for this month: %.1f", totalWithdrawals));
        addTypingEffect(progressBar2, String.format("Earning for this month: %.1f $", earningValue));
        addTypingEffect(progressBar3, String.format("Deposits for this month: %.1f", totalDeposits));

        // Configure Line Chart - unchanged
        ...
        // Fetch total balance for Bar Chart percentage calculation - unchanged
        ...
        // Bar Chart using database data with percentage of daily transactions relative to total balance - unchanged
        List<Object[]> dailyData = database_BankSystem.getDailyTransactions(username);

        // Use the existing 'today' variable
